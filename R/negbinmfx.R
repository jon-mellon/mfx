negbinmfx <-
function(formula, data, atmean=TRUE, robust=FALSE, 
                             clustervar1=NULL, clustervar2=NULL){
  res = negbinmfxest(formula, data, atmean, robust, clustervar1, clustervar2)
  
  est = NULL
  est$mfxest = cbind(dFdx = res$mfx$mfx,
                     StdErr = res$mfx$se,
                     z.value = res$mfx$mfx/res$mfx$se,
                     p.value = 2*pt(-abs(res$mfx$mfx/res$mfx$se), df=res$fit$df.residual))
